...
class PointNet2MSG(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['point_batch_idx'] = batch_idx
        return batch_dict

class PointNet2MSG_NOFP(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINTS.__len__()):
            ...
            skip_channel_list.append(channel_out)
            channel_in = channel_out
        self.npoints = self.model_cfg.SA_CONFIG.NPOINTS[-1]
    ...
    def forward(self, batch_dict):
        ...
        xyz_batch_cnt = xyz.new_zeros(batch_size).int()
        batch_idx_nofp = torch.zeros(
                self.npoints*batch_size,1,
                dtype=batch_idx.dtype,
                device=batch_idx.device)
        for bs_idx in range(batch_size):
            xyz_batch_cnt[bs_idx] = (batch_idx == bs_idx).sum()
            batch_idx_nofp[self.npoints*bs_idx:self.npoints*(bs_idx+1)] = bs_idx
        ...
        batch_dict['point_features'] = point_features.view(-1, point_features.shape[-1])
        batch_dict['point_coords'] = torch.cat((batch_idx_nofp.float(), l_xyz[-1].view(-1, 3)), dim=1)
        batch_dict['point_batch_idx'] = batch_idx_nofp.squeeze(1)
        return batch_dict
